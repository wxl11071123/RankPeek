    ...
    private List<DeepSeekChatMessage> buildPostgameMessages(PostgameAnalysisRequest request) {
        ...
        String task = "praise".equals(mode)
                ? """
                        ...
                        判断方向：
                        1. 如果【你】表现好并且赢了：大方鼓励和吹嘘，可以写得爽一点；强调这是有明确贡献的胜利，不是混赢。
                        ...
                        4. 如果【你】数据不好看且输了：先安慰并降低压力，帮用户甩锅，不让用户吃一点压力；承认有可改进点也要轻描淡写，把失败解释成多因素结果，可以从阵容、队友位置节奏、对手强势点或资源交换里找外部原因，但不要辱骂、阴阳怪气或攻击其他玩家。
                        输出规则：
                        - headline 像一句醒目的夸夸标题。
                        - headline 不要使用固定模板“这把真不能全怪你”；赢局不要用“不背锅”“不能怪你”类标题，必须按英雄、位置、胜负和表现自拟。
                        - body 写 220 到 450 个中文字符。
                        - 必须提到【你】的英雄和位置。
