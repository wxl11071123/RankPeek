...
EXPECTED_FLYWAY_VERSION="${RANKPEEK_SMOKE_EXPECTED_FLYWAY_VERSION:-10}"
...
EXPECTED_INITIAL_ADMIN_ENABLED="${RANKPEEK_SMOKE_EXPECT_INITIAL_ADMIN_ENABLED:-}"
...
admin_diagnostics_expectations_configured() {
  [[ -n "${RANKPEEK_SMOKE_EXPECTED_FLYWAY_VERSION+x}" \
    ...
    || -n "${RANKPEEK_SMOKE_EXPECT_INITIAL_ADMIN_ENABLED+x}" \
...
INITIAL_ADMIN_ENABLED="$(printf '%s' "$DIAGNOSTICS_BODY" | jq -r '.data.configuration.initialAdminEnabled')"
...
assert_optional_bool "initialAdminEnabled" "$EXPECTED_INITIAL_ADMIN_ENABLED" "$INITIAL_ADMIN_ENABLED"
