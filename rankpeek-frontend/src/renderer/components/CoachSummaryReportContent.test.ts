...
test('coach summary report analysis section hides champion advice cards', () => {
  ...
  assert.match(source, /findingText\(finding\)/)
  assert.match(source, /finding\.evidence/)
  assert.match(source, /finding\.reasoning/)
  assert.doesNotMatch(source, /finding\.advice\s*\|\|\s*finding\.reasoning\s*\|\|\s*finding\.evidence/)
  ...
  assert.doesNotMatch(source, /counter-increment:\s*report-item/)
  assert.doesNotMatch(source, /counter\(report-item/)
})
...
test('coach summary report overview keeps facts and hero win-rate figure', () => {
  ...
  assert.match(source, /COACH_SUMMARY_HERO_ICON_FALLBACK_IDS/)
  assert.match(source, /heroIconChampionId\(hero\)/)
  assert.match(source, /heroDetailLine\(hero\)/)
  assert.match(source, /hero\.championCanonicalName/)
  assert.match(source, /hero\.championDisplayName/)
...
test('coach summary report hero win-rate bars are rectangular and reveal values on hover or focus only', () => {
  ...
  assert.match(source, /hero-win-rate-games/)
  assert.match(source, /formatHeroChartLabel\(hero\)/)
  assert.match(source, /class="hero-win-rate-column"[\s\S]*:style="\{ '--hero-win-rate': `\$\{clampPercent\(hero\.winRate\)\}%` \}"/)
  assert.match(source, /:aria-label="formatHeroChartLabel\(hero\)"/)
  assert.match(source, /:title="formatHeroChartLabel\(hero\)"/)
  ...
  assert.match(source, /<span class="hero-win-tooltip">[\s\S]*\{\{ formatPercent\(hero\.winRate\) \}\}[\s\S]*\{\{ hero\.games \}\} 场[\s\S]*<\/span>/)
  assert.match(source, /<span class="hero-win-rate-games">\{\{ hero\.games \}\} 场<\/span>/)
...
test('coach summary report increases information density and prose readability', () => {
  const source = readFileSync(contentUrl, 'utf8')

  assert.match(source, /heroDetailLine\(hero\)/)
  assert.match(source, /heroRecordText/)
  assert.match(source, /heroAverageKdaText/)
  assert.match(source, /hero\.wins/)
  assert.match(source, /hero\.losses/)
  assert.match(source, /hero\.averageKda/)
  assert.match(source, /\.section-heading h2\s*\{[\s\S]*font-size:\s*28px/)
  assert.match(source, /\.section-summary,\s*\n\.final-summary\s*\{[\s\S]*font-size:\s*24px/)
  assert.match(source, /\.finding-copy\s*\{[\s\S]*font-size:\s*24px/)
  assert.doesNotMatch(source, /\.fact-sub,\s*\n\.hero-main p,\s*\n\.hero-metrics span,\s*\n\.finding-item p\s*\{/)
  assert.match(source, /\.hero-main h3\s*\{[\s\S]*font-size:\s*15px/)
  assert.match(source, /\.hero-main p\s*\{[\s\S]*font-size:\s*13px/)
  assert.match(source, /\.hero-metrics span\s*\{[\s\S]*font-size:\s*13px/)
  assert.match(source, /\.hero-win-tooltip strong\s*\{[\s\S]*font-size:\s*13px/)
  assert.match(source, /\.hero-win-rate-games\s*\{[\s\S]*font-size:\s*12px/)
  assert.match(source, /\.overview-layout\s*\{[\s\S]*gap:\s*18px/)
  assert.match(source, /\.hero-win-rate-chart\s*\{[\s\S]*min-height:\s*240px/)
})
...
test('coach summary report applies serif prose only to AI generated copy', () => {
  ...
  assert.match(source, /<p class="finding-copy ai-report-prose">\{\{ findingText\(finding\) \}\}<\/p>/)
  assert.match(source, /<p class="final-summary ai-report-prose">\{\{ closingSummary \}\}<\/p>/)
