...
test('AI analysis history renders report cards that promote report content over engineering metadata', () => {
  ...
  assert.doesNotMatch(source, /report-meta/)
...
test('AI analysis history renders saved praise runs with the match-page praise note style', () => {
  const source = readViewSource()
  const localAiAnalysis = readRendererFile('services/localAiAnalysis.ts')

  assert.match(localAiAnalysis, /postgamePraise\?: PostgameAiPraiseResult/)
  assert.match(source, /result\.output\.postgamePraise/)
  assert.match(source, /class="report-praise-card"/)
  assert.match(source, /class="report-praise-headline"[\s\S]*result\.output\.postgamePraise\.headline/)
  assert.match(source, /class="report-praise-body"[\s\S]*result\.output\.postgamePraise\.body/)
})
