...
test('match history top bar uses fixed performant styling', () => {
  ...
  const stickyOverrideRule = extractRule(source, '.page-shell.surface-glow {')
  ...
  assert.match(pageShellRule, /position:\s*sticky/)
  assert.match(pageShellRule, /top:\s*16px/)
  ...
  assert.match(stickyOverrideRule, /position:\s*sticky/)
  assert.match(stickyOverrideRule, /z-index:\s*80/)
  ...
  assert.match(source, /\.history-shell:hover,\n\.history-shell\.surface-glow\[data-near-glow='true'\],[\s\S]*\.state-card\.surface-glow\[data-near-glow='true'\] \{[\s\S]*border-color:\s*rgba\(148, 211, 255, 0\.28\)/)
...
test('match history select filters have an activated hover glow state', () => {
  ...
  assert.match(filterControlHoverRule, /border-color:\s*var\(--match-control-border\)/)
  ...
  assert.match(filterControlActiveRule, /background:[\s\S]*var\(--match-control-bg-active\)/)
